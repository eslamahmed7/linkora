    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 sm:p-8">
      <div className="bg-[#0E0E18] border border-neutral-800 rounded-3xl max-w-3xl w-full shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-neutral-800">
          <div>
            <h2 className="text-xl font-extrabold text-white">Choose a Template</h2>
            <p className="text-sm text-neutral-500 mt-0.5">Pick a base design — you can fully customize it after</p>
          </div>
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-[#0E0E18] border border-neutral-800 rounded-3xl max-w-md w-full shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-neutral-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-500">Using: <span className="text-indigo-400">{template.name}</span></p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-neutral-800 text-neutral-500 hover:text-white transition-colors">
            <X className="w-5 h-5" />
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-bold text-neutral-400 mb-1.5 uppercase tracking-wider">Card Name *</label>
            <input
              type="text"
              placeholder="e.g. My Smart Business Card"
              value={form.title}
          ...
          <div>
            <label className="block text-xs font-bold text-neutral-400 mb-1.5 uppercase tracking-wider">Description</label>
            <textarea
              placeholder="Where will you use this card?"
          ...
          <div>
            <label className="block text-xs font-bold text-neutral-400 mb-1.5 uppercase tracking-wider">Destination Page</label>
            <select
              value={form.pageId}
            ...
            >
              {pages.length === 0
                ? <option value="">Create a page first</option>
    ...
    <div className={`bg-white dark:bg-neutral-950 rounded-2xl border transition-all hover:shadow-lg flex flex-col overflow-hidden ${card.isActive ? 'border-neutral-200 dark:border-neutral-800' : 'border-neutral-100 dark:border-neutral-900 opacity-60'}`}>
      ...
      <div className="p-5 bg-gradient-to-br from-neutral-800 via-neutral-900 to-neutral-950 relative overflow-hidden h-36 flex flex-col justify-between">
        ...
        <div className="flex items-center justify-between z-10">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold tracking-widest text-neutral-400 uppercase">NFC Card</span>
          </div>
        ...
        <div className="z-10">
          ...
          <p className="text-[10px] text-neutral-500 mt-0.5 line-clamp-1">{card.description || 'No description'}</p>
        </div>
        <div className="flex items-center justify-between z-10 border-t border-white/5 pt-2">
          <span className="text-[9px] text-neutral-600 font-mono">
            {card.tagId ? `UID: ${card.tagId.substring(0, 12)}...` : 'Tag not linked'}
          </span>
          {hasDesign && (
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-center justify-between text-xs">
          <span className="text-neutral-500 font-medium">Redirects to:</span>
          {linkedPage ? (
            ...
            <span className="text-neutral-500 italic">No Page Linked</span>
          )}
        </div>
        ...
        >
          ...
          {hasDesign ? 'Edit Design' : 'Create Design'}
        </button>
        ...
        <div className="flex items-center justify-between pt-1 border-t border-neutral-100 dark:border-neutral-900 gap-1">
          ...
          <div className="flex items-center gap-1">
            <button onClick={onToggle} className={`p-2 rounded-lg transition-colors ${card.isActive ? 'text-green-500 hover:bg-green-50 dark:hover:bg-green-950/20' : 'text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800'}`} title={card.isActive ? 'Deactivate' : 'Activate'}>
...
export function NFCPage() {
  ...
  const handleCreateCard = async (data: { title: string; description: string; pageId: string }) => {
    try {
      ...
      notification.success('Card created! Opening designer...')
    } catch {
  ...
  const handleSaveDesign = async (design: CardDesign) => {
    ...
    try {
      ...
      notification.error('Failed to save design')
    }
  }
  ...
  const handleToggleActive = async (id: string) => {
    try {
      ...
      notification.success(card.isActive ? 'Card activated' : 'Card deactivated')
    } catch {
      notification.error('Failed to update card status')
    }
  }
  ...
  const handleSimulateWrite = async () => {
    ...
    try {
      ...
      setSimSuccess(true)
      notification.success('NFC tag linked!')
    } catch {
    ...
    <div className="max-w-6xl mx-auto py-6 space-y-8 px-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-500 dark:text-neutral-400 mt-1.5 ml-1">
            Create, customize, and manage your premium NFC business cards.
          </p>
        </div>
        <button
      ...
      {!isLoading && cards.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: 'Active', value: activeCards, icon: Power, color: 'green' },
            { label: 'Linked Tags', value: cards.filter(c => c.tagId).length, icon: Wifi, color: 'purple' },
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-3">
          ...
          <p className="text-neutral-500">Loading NFC cards...</p>
        </div>
      ) : cards.length === 0 ? (
        <div className="bg-white dark:bg-neutral-950 rounded-3xl border-2 border-dashed border-neutral-200 dark:border-neutral-800 p-16 text-center max-w-xl mx-auto space-y-6">
          ...
          <div>
            <h3 className="text-xl font-extrabold text-neutral-900 dark:text-white">No NFC Cards Yet</h3>
            <p className="text-neutral-500 text-sm mt-1 max-w-sm mx-auto">
              Create your first premium NFC business card. Choose a template and customize every detail.
            </p>
          </div>
          <button
            onClick={() => setShowTemplatePicker(true)}
      ...
      {editingCard && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-3xl max-w-md w-full shadow-2xl p-6 relative">
            ...
            <h3 className="text-xl font-extrabold text-neutral-900 dark:text-white mb-5">Edit Card</h3>
            <div className="space-y-4">
              {[
                { label: 'Card Name', key: 'title' as const, placeholder: 'My Smart Card', type: 'text' },
                { label: 'Description', key: 'description' as const, placeholder: 'Where will you use this?', type: 'textarea' },
              ...
              <div>
                <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1.5">Destination Page</label>
                <select
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={handleSaveEdit} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 rounded-xl transition-all">Save</button>
                <button onClick={() => setEditingCard(null)} className="flex-1 bg-neutral-100 dark:bg-neutral-900 hover:bg-neutral-200 dark:hover:bg-neutral-800 text-neutral-700 dark:text-neutral-300 font-bold py-2.5 rounded-xl transition-all">Cancel</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-3xl max-w-md w-full shadow-2xl overflow-hidden">
            <div className="p-5 border-b border-neutral-100 dark:border-neutral-900 flex items-center justify-between">
              <div>
                <h3 className="font-extrabold text-lg text-neutral-900 dark:text-white">NFC Tag Writer</h3>
                <p className="text-xs text-neutral-500 mt-0.5">Simulate linking a physical NFC chip</p>
              </div>
            ...
            <div className="p-6 space-y-6">
              <div className="relative flex flex-col items-center justify-center bg-neutral-50 dark:bg-neutral-900 rounded-2xl py-10 border border-neutral-200/50 dark:border-neutral-800/50">
                <div className={`w-32 h-20 bg-gradient-to-br from-neutral-800 to-neutral-950 rounded-xl flex flex-col items-center justify-center gap-1 text-white border border-neutral-700 relative z-10 transition-all ${isSimulating ? 'animate-pulse scale-105' : ''}`}>
                  ...
                  <span className="text-[10px] text-neutral-400">Card Target</span>
                </div>
                <div className={`mt-6 flex flex-col items-center gap-1.5 transition-all duration-700 ${isSimulating ? '-translate-y-2' : ''}`}>
                  ...
                  <span className="text-[10px] text-neutral-400">Place phone near card</span>
                </div>
                ...
                {simSuccess && (
                  <div className="absolute inset-0 bg-white/95 dark:bg-neutral-950/95 flex flex-col items-center justify-center gap-2 z-20 rounded-2xl">
                    ...
                    <span className="font-bold text-neutral-900 dark:text-white">Tag Linked!</span>
              ...
              {!simSuccess ? (
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-neutral-500 mb-1.5">NFC Tag UID</label>
                    <div className="flex gap-2">
                      <input
                        type="text" value={simTagId}
                        onChange={e => setSimTagId(e.target.value)}
                        placeholder="e.g. 04:DE:6F:A2"
                  ...
                  >
                    {isSimulating ? 'Writing NFC...' : 'Start Simulation'}
                  </button>
                </div>
              ) : (
                <button onClick={() => setShowSimulator(false)} className="w-full bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 font-bold py-3 rounded-xl">Done</button>
              )}
            </div>
          </div>
        </div>
