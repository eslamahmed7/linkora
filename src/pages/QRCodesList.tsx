...
export function QRCodesListPage() {
  ...
  const fetchQRCodes = async () => {
    try {
      ...
      notification.error('Failed to load QR codes')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      notification.success('QR code deleted successfully')
    } catch (error) {
      notification.error('Failed to delete QR code')
    }
  }
  ...
  const handleToggle = async (id: string) => {
    try {
      ...
      notification.success(
        response.qrCode.isActive ? 'QR code activated' : 'QR code deactivated'
      )
    } catch (error) {
      notification.error('Failed to update QR code')
    }
  }

  const handleCopyUrl = (url: string) => {
    navigator.clipboard.writeText(url)
    notification.success('QR code URL copied to clipboard')
  }
                    ...
                    <div className="flex flex-col">
                      ...
                      {qr.customization?.dataType === 'landing-page' && pages.find(p => p.id === qr.pageId) && (
                        <span className="inline-flex items-center gap-1 mt-1 text-[10px] font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/50 px-2 py-0.5 rounded w-max">
                          ...
                          Linked to: {pages.find(p => p.id === qr.pageId)?.title}
                        </span>
                      )}
                    </div>
                  </td>
