  ...
  return (
    <div className="p-4 lg:p-6 space-y-6 max-w-[1400px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-extrabold text-neutral-900 dark:text-white tracking-tight">Backups</h1>
          <p className="text-sm text-neutral-500 mt-0.5">{completedCount} backups · {formatBytes(totalSize)} total</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { icon: Database, label: 'Total Backups', value: backups.length, color: 'text-accent-600' },
          { icon: HardDrive, label: 'Total Size', value: formatBytes(totalSize), color: 'text-blue-600' },
          { icon: Clock, label: 'Last Backup', value: backups.length > 0 ? new Date(backups[0].created_at).toLocaleDateString() : 'Never', color: 'text-green-600' },
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-neutral-500 text-sm">No backups yet</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                  <th className="text-left px-5 py-3 text-[11px] font-bold text-neutral-500 uppercase tracking-wider">Name</th>
                  <th className="text-left px-5 py-3 text-[11px] font-bold text-neutral-500 uppercase tracking-wider">Type</th>
                  <th className="text-left px-5 py-3 text-[11px] font-bold text-neutral-500 uppercase tracking-wider">Size</th>
                  <th className="text-left px-5 py-3 text-[11px] font-bold text-neutral-500 uppercase tracking-wider">Status</th>
                  <th className="text-left px-5 py-3 text-[11px] font-bold text-neutral-500 uppercase tracking-wider">Created</th>
                </tr>
              </thead>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-2xl max-w-md w-full shadow-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-extrabold text-neutral-900 dark:text-white">New Backup</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-neutral-500 mb-1 block">Backup Name (optional)</label>
                <input type="text" value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. pre-update backup"
